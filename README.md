...
② Attack angle and direction: Pressing the `W` and `S` keys respectively increases and decreases the launch angle (maximum 80°, minimum -60°, it will automatically switch direction if exceeded, or you can press the `Q` key to switch direction).
③ Power: Pressing the `+` (plus) and `-` (minus) keys respectively increases and decreases the power (minimum 5, maximum 23).
④ Launch: Press the `Enter` key to launch. **Please pay attention to the wind direction and level, and adjust the angle and power accordingly.**
⑤ Skills: Click on the `Current Skill` or press the `Tab` key to view the skill help. The number keys correspond to the respective skills (`0` to `9`). For additional skills, you need to first press the `*` (asterisk) key or `/` (slash) key to switch the skill group. Press the `Esc` (escape) key or `Backspace` (backspace) key to cancel the selected skill.
