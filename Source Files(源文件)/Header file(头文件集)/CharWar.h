/*此文件包含了除了主函数之外的所有的功能函数和所有要用到的全局变量

This file contains all the functions except the main function and all the global variables to be used
*///////////////
...
const int map_wide = 640 * 1.5;	//地图or窗口大小，游玩时可以自行更改
const int map_high = 480 * 1.5; //很多字体呈现没有做适配，请不要随意改成更小尺寸
...
int Play = 1, v0 = 10, Sigma = 45, Players = -1, selfround, Storge = 1;	//公共变量，当前玩家，玩家数，自己回合，收数据, 局域网或者本地, 目前视图, 自动使用技能
int View = -1;//veiw=-1>>主菜单；view=-2>>设置界面；view=-3>>个人界面；view=-4>>抽卡界面；view=-5>>开始游戏界面；view=0>>>>对战界面；view=1；技能界面view=2;
...
int keystr[] = { '!', '!', '!', '!' };
int Charkey[] = { 65, 68, 87, 83, 81, 107, 109, 106, 111, 110, 70};	//A，D， W， S， Q， +， -， *， /， .， F
...
void DrawTitle()
{
	...
	int txtleft = (getwidth() - txtwide) / 2;
	int txttop = (getheight()) / 2 - textheight(strtemp) * 10;
	...
	txtleft = (getwidth() - textwidth(strtemp)) / 2;
	...
	for (int i = 1, j = 0; i <= listnum; i++) {
		...
		if (i % 2 == 0 && i < listnum) {
			...
			txtleft = (getwidth() - textwidth(strtemp)) / 2;
			txttop = (getheight()) / 2 - textheight(strtemp) * (10 - j * 3);
		}
	}
...
void FallDown(float Speed = 10)
{
		...
		getpixel(数据[10][9] + 8, 数据[10][10] + 14 + 2) != Block)
	{
		...
		if (PlayersData[10][10] >= getheight() / 2 + 20)
		{
			PlayersData[10][7] = 0;
	...
	for (int i = 1; i <= Players; i++)
		if (PlayersData[i][9] > 0)
				...
				getpixel(PlayersData[i][1] + 8, PlayersData[i][2] + 16 + 2) != Block)
			{
				...
				if (PlayersData[i][2] >= getheight() / 2.0)
					if (View == 0)
						while (1)
						{
							...
							PlayersData[i][2] -= 5.0;
							Flyyy++;
							if (PlayersData[i][2] <= -getheight() / 2.0 - 20)
							{
								PlayersData[i][1] = RANDOM(-getwidth() / 2.0 - 30, getwidth() / 2.0 - 30);
								PlayersData[i][2] = -getheight() / 2.0 - 20;
...
void drawUI()
{
	...
	if (View == -1 || View == -5)
	{
		Prime_RECT();
		setorigin(0, 0);
		TEXTOUT(L"C h a r W a r", getwidth()/2-270, 50, 40, L"华文细黑", 255, 200, 200);
	...
	if (View == 0)
	{
		...
		button(L"当前技能:", -UIleft - 210, UItop + 55, 65, 20, 3, L"等线", 0, 0, 100, 20, 200, 255);
		setorigin(getwidth() / 2, getheight() / 2);
		...
		/* {
			if (Players == 2)
				Prime_RECT(UIleft + 40 + 150 + 5, UItop, UIleft + 40 + 150 + 85, UItop + 50, 0, 0, 0, 255, 255, 254);
			else
				Prime_RECT(UIleft + 40 + 150 + 5, UItop, UIleft + 40 + 150 + 85, UItop + 70, 0, 0, 0, 255, 255, 254);

			if (Players >= 4) {
				Prime_RECT(UIleft + 40 + 150 + 85 - 10, UItop + 3, UIleft + 40 + 150 + 85 - 3, UItop + 70 - 3, 0, 0, 0, 255, 255, 254);
				float SlideH = 3 * 60 / Players; //60->框的高度， 3->能显示的玩家数

				Prime_RECT(UIleft+40+150+85-8, UItop+5, UIleft+40+150+85-5, UItop + 5+ SlideH, 0,0,0);
			}
			for (int i = 1; i <= Players; i++) {
				float Hei = UItop + (i - 1) * 20 + 5;
				if (UItop + (1 - 1) * 20 + 5 <= Hei && Hei <= UItop + (3 - 1) * 20 + 5) {
					swprintf(strtmp, L"%s: %.0f%%", PlayerChar[i - 1], PlayersData[i][9] / 4800.0 * 100);
					TEXTOUT(strtmp, UIleft + 40 + 150 + 7, Hei, 3, L"幼圆", PlayersData[i][3], PlayersData[i][4], PlayersData[i][5]);
				}
			}
		}*/
	   ...
	   //TEXTOUT(L"当前技能:", -UIleft-210, UItop+55, 4, L"等线",	 0,0,100);
		...
		swprintf(strtmp, L"ROUND: %.0f", PlayersData[10][1]);
		TEXTOUT(strtmp, -textwidth(strtmp)/2, UItop + 5, 7, L"Technic", 150, 150, 240);

		Prime_RECT(28, UItop + 33, 28+25, UItop + 33+25, 255, 255, 255, 0, 0, 0);
		TEXTOUT(PlayerChar[Play-1], 30, UItop + 35, 6, L"等线", 数据[Play][3], 数据[Play][4], 数据[Play][5]);
		...
		for (int i = 0; i < 9; i++)
			if (thresholds[i] <= PlayersData[10][5] && PlayersData[10][5] <= thresholds[i + 1]) {
				...
				TEXTOUT(strtmp, -textwidth(strtmp)/2-25, UItop + 30, 3, L"幼圆", 0, 150, 230);
				break;
			}

		swprintf(strtmp, L"%.2f%%", fabs(PlayersData[10][5] * 25));//风速
		TEXTOUT(strtmp, -textwidth(strtmp) / 2-25, UItop + 45, 3, L"幼圆", 0, 150, 230);
	...
	//画地雷
	if (View == 0 && PlayersData[10][1] <= Players) //回合数小于玩家数
...
void Ranmap()
{
	...
	for (x = 0; x < getwidth(); x++)
	{
		...
		y += random;
		line(x, y, x, getheight());
	}
	x = RANDOM(20, getheight() / 2);
	y = RANDOM(20, getheight() / 2);
	setfillcolor(Block);
...
void LoadMap() {
	setorigin(0, 0);
	loadimage(&CharWar, PATHcTow(PATH), getwidth(), getheight(), true);
	putimage(0, 0, &CharWar);

	setorigin(getwidth() / 2, getheight() / 2);   //set world center  世界以我为中心
...
void Slp() 
{
	TEXTOUT(L"程序已挂起(按键唤醒)", -100, -getheight() / 2 + 60, 5);
...
HRESULT CharWindows(int ToSend)
{
	...
	HWND hWndDows = FindWindow(NULL, L"CharDows");
	...
	if (hWndDows == NULL) {
		...
		SetForegroundWindow(hWndDows);

	return 1;
}

//技能帮助菜单，View=2时不返回主界面也不能退出
int SkillMenu()
{
	TCHAR strrr[50]{ 0 };
...
{"固定伤害r850，伤害半径p+5，在当前攻击方向放一只羊（移动方向p随p机），按下b回b车后向上方起飞，之后方向操控(bW或bA逆时针旋转)，超过p15p秒后(自动起飞或)消失，撞到地图后爆炸(会穿过玩家)"},//bwd是吧bA是吧
...
{"在附近架一台o500血的机炮，当有敌人位置p大p幅变化时锁定发射p6p枚子炮(直线，每颗r35伤害，结束后本回合不再射击，换回合后会再次有空)，血条清空时会爆炸，基础伤害r220，伤害半径p-1"},
{"固定伤害r250，伤害半径p+1，p直p线投出，被击中玩家下回合不能使用技能（但是可以o跳o回o合和使用r血r包），不能p移p动，伤害p8p折（无论是什么技能）"},
{"固定伤害r380，伤害半径p+2，从上往下钻井，直至打中目标或钻井时间耗尽"},
{"基础伤害r420，指定追踪处，可按bShift键重新指定光标处。炮弹抛出p1秒后以p直p线飞向光标处"},
...
{"基础伤害r400，向预瞄线抛出，具有球的特性，按bShift可调整爆炸延迟"}
	...
	setorigin(getwidth() / 2, getheight() / 2);   //set world center  世界以我为中心
	int UIleft = -getwidth() / 2, UItop = -getheight() / 2;
	int Cur = 0, CurTemp = 1;
	int AutoCur = 9;
	short int mark = 1; //演示技能与否
	char strtemp[1024]{};
	...
	if (View != 2) {
		button(L"退出", -UIleft - 50, UItop, 50, 24);
		button(L"技能演示", UIleft + 355, -UItop - 290, 80, 25, 4, L"等线", 0, 0, 100, 20, 200, 255);
		button(L"选用该技能", UIleft + 355 + 80 + 5, -UItop - 290, 100, 25, 4, L"等线", 255, 200, 120, 120, 200, 255);
	}else
		button(L"演示", UIleft + 255, UItop + 3, 50, 20, 4, L"等线", 120, 0, 0, 120, 200, 255);
	...
	while (lopoppop == 1)
	{
		...
		else
		{
			...
			case WM_LBUTTONDOWN: {
				//自动到顶
				if (0 < m.x && m.x < 310) {
					AutoCur = -(m.y + 15 - (-576/2) - Cur * 30) / 30 + 12; //576是窗口高度，这个不能改
					if (0 < m.y && m.y < 25)
						AutoCur = 1;
				}
			...
			case WM_KEYDOWN:
			{
				if (View != 2 && m.vkcode == VK_ESCAPE) lopoppop = 0;
		...
		if (CurTemp != Cur) {
			Prime_RECT(UIleft, UItop+26, UIleft + 310, -UItop);


			if (( - 1 <= Cur && Cur <= 1) || Cur == -8) {
				if (mark == 1){
					if (View != 2) {
						LINE(UIleft + 355, -UItop - 290, UIleft + 355 + 80, -UItop - 290 + 25, { 255,0,0 });
						LINE(UIleft + 355, -UItop - 290 + 1, UIleft + 355 + 80 - 1, -UItop - 290 + 25, { 255,0,0 });
						LINE(UIleft + 355 + 1, -UItop - 290, UIleft + 355 + 80, -UItop - 290 + 25 - 1, { 255,0,0 });
					}else {
						LINE(UIleft + 255, UItop + 3, UIleft + 255 + 50, UItop + 3 + 20, { 255,0,0 });
						LINE(UIleft + 255, UItop + 3 + 1, UIleft + 255 + 50 - 1, UItop + 3 + 20, { 255,0,0 });
						LINE(UIleft + 255 + 1, UItop + 3, UIleft + 255 + 50, UItop + 3 + 20 - 1, { 255,0,0 });
					}
				}
				mark = -1;
			}
			...
			for (int i = 0; i < 24; i++)
			{
				int TxY = UItop + 30 + (Cur + i) * 30;
				if (UItop+25 < TxY && TxY < -UItop) {
					if (i <= 9)
					...
					TEXTOUT(strrr, UIleft + 2 + 28 - textwidth(strrr) / 2, TxY, 5, L"等线", 200, 120, 90);
					swprintf(strrr, L"%d", i - 1);
					TEXTOUT(strrr, UIleft + 2 + 64 + 16 - textwidth(strrr) / 2, TxY, 5, L"等线", 70, 190, 160);
					TEXTOUT(PointsStr[i], UIleft + 2 + 112 + 16 - textwidth(PointsStr[i]) / 2, TxY, 5, L"等线", 80, 80, 255);

					if (PlayersData[Play + 10][i] < 0)
						swprintf(strrr, L"无限");
					else
						swprintf(strrr, L"%.0f", PlayersData[Play + 10][i]);
					TEXTOUT(strrr, UIleft + 2 + 160 + 16 - textwidth(strrr) / 2, TxY, 5, L"等线", 180, 30, 255);
					TEXTOUT(SkillNames[i], UIleft + 2 + 208, TxY, 5, L"等线", 60, 80, 230);
				}
			}
...
//sprintf(strtemp, "%hd,  %hd", m.x, m.y);
//TEXTOUT(PATHcTow(strtemp), 0, 0, 3, L"幼圆", 0,0,0, 255,255,255);
