...
void playchar() {
	...
	BOOL JUmp = FALSE, VxVy = TRUE, SuaStop = TRUE;		//Jump->跳跃标志位 VxVy->vxvy重赋值标志位 SuaStop->刷新标志位
	float UIleft = -getwidth() / 2.0, UItop = -getheight() / 2.0;
	...
	if (Auto >= 0) {
		...
		else
		{
			...
			{
				MinesArry[3 * i - 2] = 1;
				MinesArry[3 * i - 1] = RANDOM(-getwidth() / 2.0 + 20, getwidth() / 2.0 - 20);
				MinesArry[3 * i] = -getheight() / 2.0 - 20;
			}
			...
			//掉雷
			TEXTOUT(L"投放地雷中...正在: ", -getwidth() / 2 + 10, -getheight() / 2 + 20, 3, L"幼圆", 50, 50, 120, 255, 255, 255);
			for (int i = 1; i <= Mines * 3; i += 3)
			{
				sprintf(str, "%d", (i - 1) / 3 + 1);
				TEXTOUT(PATHcTow(str), -getwidth() / 2 + 160, -getheight() / 2 + 20, 3, L"幼圆", 50, 50, 120, 255, 255, 255);
			...
			Prime_RECT(-getwidth() / 2 + 10, -getheight() / 2 + 20, -getwidth() / 2 + 10 + 180, -getheight() / 2 + 20 + 20);
			drawUI();
		}
	...
	while (Play != -1)
	{
		...
		//TEXTOUT(PATHcTow(str), UIleft, -UItop-20, 3, L"幼圆", 0,0,0, 255,255,255);
		...
		//	Slp();
		//	//system("pause");
		//	//TEXTOUT(L"程序已挂起(按键唤醒)", -100, -getheight() / 2 + 60, 5, L"幼圆", 255, 255, 255);
		//	LoadMap();
		//	drawUI();
		//	DrawMan();
		//	RunTime = 0;
		...
		if (selfround == Play) {
			...
			if (GetAsyncKeyState(VK_LBUTTON) & 0x8000) {
				...
				mx = mouse.x-getwidth()/2.0;
				my = mouse.y-getheight()/2.0;
		...
		//非移动时显示玩家血条
		if (FeedKey != Charkey[0] && FeedKey != Charkey[1] &&
			((GetAsyncKeyState(Charkey[10]) & 0x8000) || (GetAsyncKeyState(VK_MBUTTON) & 0x8000)))
			{
				Show++;
				
				if (Show == 1 || x0 != PlayersData[Play][1] || y0 != PlayersData[Play][2])
				{
					LoadMap();
					drawUI();
					DrawMan();
					TCHAR strtmp[50];
					int Show = 0;  //中置有奇怪的BUG

					for (int i = 1; i <= Players; i++) {
						if (PlayersData[i][9] > 0)
						{
							Show++;
							Prime_RECT(PlayersData[i][1] - 2, PlayersData[i][2] - 6, PlayersData[i][1] + 16, PlayersData[i][2] - 1, 0, 0, 0, 255, 255, 254);
							Prime_RECT(PlayersData[i][1] - 1, PlayersData[i][2] - 5, PlayersData[i][1] + PlayersData[i][9] / 320, PlayersData[i][2] - 2, 50 + int((4800 - PlayersData[i][9]) / 24), 240 - int((4800 - PlayersData[i][9]) / 20), 50);
							swprintf(strtmp, L"%.0f", PlayersData[i][9]);
							if (Show == 1)
								TEXTOUT(strtmp, 数据[i][1] + 12 - textwidth(strtmp) / 2, 数据[i][2] - 18, 1);
							else
								TEXTOUT(strtmp, 数据[i][1]+8 - textwidth(strtmp)/2, 数据[i][2] - 18, 1);
							if (PlayersData[i][10] > 0)
							{
								Prime_RECT(PlayersData[i][1] - 1, PlayersData[i][2] - 5, PlayersData[i][1] + PlayersData[i][10] / 320, PlayersData[i][2] - 2, 50, 80, 255);
								swprintf(strtmp, L"%.0f", PlayersData[i][10]);
								TEXTOUT(strtmp, 数据[i][1]+8 - textwidth(strtmp) / 2, 数据[i][2] - 28, 1, L"幼圆", 0,0,255);
							}
						}
					}
				}
				...
				{
					TCHAR strtmp[50];
					swprintf(strtmp, L"%.0f", PlayersData[10][14]);
					TEXTOUT(strtmp, 数据[10][15] - 8, 数据[10][16] - 30, 1);
	...
	FireBack:
		...
		SkillBack:	//技能循环函数头部，需要指定位置时用到
			...
			if (SkillRep <= -1 || (Skill == 21 && (SkillRep - 1) % 3 == 0 && Skill != 0))
			{
				...
				while (key != 30)
				{
					...
					if (Skill == 21) {
						if (GetAsyncKeyState(Charkey[5]) || GetAsyncKeyState(Charkey[6])) //+,-
						{
							if (GetAsyncKeyState(Charkey[5]) && PlayersData[Play][10] >= 100)
							{
								CRg++;
								Inj += 40;
								PlayersData[Play][10] -= 100;
							}

							if (GetAsyncKeyState(Charkey[6]) && CRg >= 1)
							{
								CRg--;
								Inj -= 40;
								PlayersData[Play][10] += 100;
							...
							sprintf(str, "次数:%d, 单发伤害:%.2f  ", CRg, Inj);
							TEXTOUT(PATHcTow(str), -getwidth() / 2 + 10, -getheight() / 2 + 20, 3, L"幼圆", 200, 120, 250, 255, 255, 255);
							Sleep(200);
						}
					}
			...
			case 22:
			{
				...
				{
					for (int i = 2; i <= 6; i++)
						if (GetAsyncKeyState(Charkey[i]) & 0x8000)
							Sua = 1;
			...
			if (Bomb == 1)
			{
					...
					if (PlayersData[f][9] > 0) {
						...
						if (数据[f][11] > 0 && PrDistce[f] <= 24 + r)
							if ((int)(r * 10) != -41 && (int)(r * 10) != 61 && (int)v0 != 42 &&
								(Skill <= 3 || (Skill == 4 && Inj != 550) || Skill == 7 || Skill == 19)) {
								setlinecolor(RGB(50, 60, 255));
				...
				//天姬屏幕抖动
				if (FireTemp[1] >= 500 && yp < getheight() / 2.0 + 20 && (Hittt >= 1 || Skill == 4 || Skill == 5 || Skill == 6))
				{
					setorigin(0, 0);
					...
					for (int i = 1; i <= Mines; i++)
						if (MinesArry[3 * i - 2] == 1)
						{
							...
							if (distance <= 22 + r)
							{
								...
								//显示轰炸
								if (v0 == 42) {
									TEXTOUT(L"轰炸中...正在: ", -getwidth() / 2 + 20, getheight() / 2 + 30, 3, L"幼圆", 255, 50, 120, 255, 255, 255);
									sprintf(str, "%d", times);
									TEXTOUT(PATHcTow(str), -getwidth() / 2 + 20 + 100, getheight() / 2 + 30, 3, L"幼圆", 255, 50, 120, 255, 255, 255);
								}

								TEXTOUT(L"累伤: ", -getwidth() / 2 + 20, getheight() / 2 + 10, 2, L"幼圆", 220, 60, 80, 255, 255, 255);
								...
								while (fabs(FlyXv > 0.2) || fabs(FlyYv) > 0.2)
								{
									...
									if (MineY >= getheight() / 2.0 + 15) { MinesArry[3 * i - 2] = 0; break; }
							...
							while (getpixel(MineX + 1, MineY + 14) != Block && getpixel(MineX + 11, MineY + 14) != Block)
							{
								TEXTOUT(L"雷", MineX, MineY, 1, L"幼圆", 255, 255, 255);
								MineY += 1;
								if (MineY >= getheight() / 2.0 + 20) { MinesArry[3 * i - 2] = 0; break; }
								TEXTOUT(L"雷", MineX, MineY, 1, L"幼圆", 50, 30, 60);
			...
			{
				int Flyyy = 0;
				while ((PlayersData[i][9] <= 0 || PlayersData[i][2] >= getheight() / 2.0) && PlayersData[i][2] != getheight() / 2.0 + 999)
				{
					...
					if (PlayersData[i][2] <= -getheight() / 2.0 - 20)
					{
						if (PlayersData[i][9] > 0)
						{
							...
							PlayersData[i][1] = 0;
							PlayersData[i][2] = getheight() / 2.0 + 999;
						}
		...
		if ((selfround == Play && (SkillKey == 8 || SkillKey == '*' || SkillKey == '.' || (-22 <= SkillKey && SkillKey <= 1) || ('/' <= SkillKey && SkillKey <= '9'))) || (selfround != Play && RecvBuffer[0] == '_'))
		{
			...
			{
				int Txtleft = getwidth() / 2 - 210 + 115;
				int Txttop = -getheight() / 2 + 55;
				...
				Prime_RECT(Txtleft, Txttop, getwidth() / 2, Txttop + 16);
			...
			{	//数字0-9
				...
				case 9: //字符斩
					TEXTOUT(L"字符斩！！", Txtleft, Txttop, 3, L"幼圆", 255, 0, 0, 255, 255, 255);
					Cost = 10;
					break;
				...
				if ((PlayersData[Play][8] < Cost && Cost > 0) || (PlayersData[Play][12] >= 1 && Cost >= 1) || PlayersData[Play + 10][SkillKey + 1] == 0)
				{
					if (PlayersData[Play][8] < Cost)
						button(L"点数不足", -UIleft - 210, UItop + 55, 65, 20, 3, L"幼圆", 150, 50, 50, 255, 255, 255);
					if (PlayersData[Play][12] >= 1)
						button(L"不能使用", -UIleft - 210, UItop + 55, 65, 20, 3, L"幼圆", 150, 50, 50, 255, 255, 255);
					if (PlayersData[Play + 10][SkillKey] == 0)
						button(L"次数不足", -UIleft - 210, UItop + 55, 65, 20, 3, L"幼圆", 150, 50, 50, 255, 255, 255);
					setorigin(getwidth() / 2, getheight() / 2);
		...
		if (JUmp == TRUE)
		{
			...
			if (getpixel(PlayersData[Play][1]+2, PlayersData[Play][2]-2)==Block||
				...
				getpixel(PlayersData[Play][1]+2, PlayersData[Play][2]+16+2)==Block||
				getpixel(PlayersData[Play][1]+16-2, PlayersData[Play][2]+16+2)==Block ||
				PlayersData[Play][2] >= getheight()/2.0)
				JUmp = FALSE;
